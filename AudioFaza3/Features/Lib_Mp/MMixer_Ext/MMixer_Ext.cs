...
public static class MMixer_Ext
{
    ...
    public static void s_SetStream_Main(this MMixerClass mm, MFileClass mf, string @params = "", double duration = 0)
    {
        try
        {
            string mp = mf.s_GetMp();
            const string streamId = "main_s";
            mm.StreamsAdd(streamId, null, mp, @params, out MItem stream, duration);
            mm.StreamsReorder(stream, 0, 0);
        }
        catch
        {
    ...
    public static void s_SetStream_Main(this MMixerClass mm, MMixerClass mf, string @params = "", double duration = 0)
    {
        try
        {
            string mp = mf.s_GetMp();
    ...
    public static string s_GetMp(this MMixerClass mfc)
    {
        mfc.ObjectNameGet(out string name);
        return "mp://" + name;
    }
    
    public static void s_AddAudioTrack(this MMixerClass mixer, MMixerClass track)
    {
        MLiveClass mLiveClass = new();
        mLiveClass.DeviceSet("video", "<None>", "");
        mLiveClass.DeviceSet("audio", track.s_GetMp(), "");
        mLiveClass.ObjectStart(null);
        mixer.StreamsAdd("audioSteam", mLiveClass, "", "", out _, 0);
    }
}